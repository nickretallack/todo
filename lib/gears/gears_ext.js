...
(function() {
  function _db(innerDb) {
    ...
    this.selectAllAsArrays = function(sqlStatement, argArray) {
      ...
      this.raw_insert = function(table, data){
      ...
      this.raw_update = function(table, keys, updates){
      ...
      this.raw_delete = function(table, keys) {
          db.run("delete from "+table+" where "+db.quote_equals(keys, ' and '))
      }
      
      this.raw_get = function(table, keys){
          var results = db.raw_get_all(table, keys)
          if (results.length) return results[0]
          else return null
      }
      
      this.raw_get_all = function(table, keys){
      ...
      this.update = function(table, keys, updates){
          if (table == 'item') {
              ...
              db.transaction(function(db) {
                  db.raw_update('item', row_key, item_updates.details)
                  db.raw_update('item_text', row_key, item_updates.texts)
              })
          } else db.raw_update(table, keys, updates)
      }
      
      this.delete_ = function(table, keys){
          if (table == 'item') {
              var rowid = db.selectSingle('select rowid from item where '+db.quote_equals(keys, ' and '))
              var row_key = {rowid:rowid}

              db.transaction(function(db) {
                  db.raw_delete('item', row_key)
                  db.raw_delete('item_text', row_key)
              })
          } else db.raw_delete(table, keys)
      }
      
      this.get = function(table, keys) {
          var results = this.get_all(table, keys)
          if (results.length) return results[0]
          else return null
      }
      
      this.get_all = function(table, keys) {
          if (table == 'item') {
              var query = "select item_text.text, item_text.note, item.* \
                    from item join item_text on item.rowid = item_text.rowid"
              if (keys && !_.isEmpty(keys)) query += " where "+db.quote_equals(keys)
              return db.selectAll(query);
          } else return db.raw_get(table, keys)
      }
